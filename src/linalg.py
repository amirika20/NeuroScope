...
def top_hessian_eigenpair(
    ...
    """
    Estimate the top Hessian eigenvalue/eigenvector via power iteration.
    IMPORTANT: Runs with gradients enabled internally, but params are restored.
    Args:
        model, loss_fn, x_batch, y_batch: define the scalar loss.
        iters: # of power iterations (10–30 is typical for small nets).
        init_vec: optional starting vector (flattened).
    Returns:
        (lambda_max, v_flat) where v_flat is a unit vector (flattened).
    """
    device = next(model.parameters()).device
    params = [p for p in model.parameters() if p.requires_grad]

    # Build computation graph *once* for HVP
    ...
    for _ in range(iters):
        # Need graph for hvp, so no @torch.no_grad() here
        hv = hvp(loss, params, v)  # flattened
        hv_detached = hv.detach()  # safe reuse without graph bloat
        # Rayleigh quotient (approx eigenvalue)
        lam = float((v * hv_detached).sum().item())
        v = _normalize(hv_detached)

    # One last Rayleigh quotient with final v
