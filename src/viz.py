...
def make_anim(
    ...
    Assemble the animation. Each frame:
      ...
      - Updates loss panel with training loss and generalization MSE.
    ...
    # Generalization mask: outside training interval (1D) or outside training box (2D)
    if cfg.input_dim == 1:
        out_mask_np = (X_vis[:, 0] < cfg.x_min) | (X_vis[:, 0] > cfg.x_max)
    else:
        out_mask_np = (
            (X_vis[:, 0] < cfg.x1_min) | (X_vis[:, 0] > cfg.x1_max) |
            (X_vis[:, 1] < cfg.x2_min) | (X_vis[:, 1] > cfg.x2_max)
        )
    ...
    def init():
        ...
        if cfg.input_dim == 1:
            ...
            # nothing to clear; we redraw the learned surface in update()
            pass
        info_text.set_text("")
    ...
    def update(_frame_idx: int):
        ...
        # ---- Generalization error on OOD region ----
        if has_outside:
            yp = y_pred_vis.index_select(0, out_idx_t)
            yt = y_true_vis_t.index_select(0, out_idx_t)
            gen_mse = torch.mean((yp - yt) ** 2).item()
        else:
            gen_mse = float("nan")
        ...
        if cfg.input_dim == 1:
            ...
            # Remove the previous Poly3DCollection (if any), then draw a new one
            sp = H.get("surf_pred", None)
            if sp is not None:
                try:
                    sp.remove()
                except Exception:
                    # If it was already cleaned up by the backend, ignore
                    pass
        ...
        # In 2D the learned surface is a Poly3DCollection (not blit-friendly), so skip it here
